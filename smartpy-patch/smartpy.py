...
def pretty_line_no():
    ...
    if window.in_browser:
        return (
            '<button class="text-button" onclick="showLine(%i)">(line %i%s)</button>'
...
class Expr:
    ...
    def get(self, item, default_value=None, message=None):
        if default_value is not None:
            return Expr(
                "getItemDefault",
                [self, spExpr(item), spExpr(default_value), get_file_line_no()],
            )
        if message is not None:
            return Expr(
                "getItemMessage",
                [self, spExpr(item), spExpr(message), get_file_line_no()],
            )
    ...
    def match(self, constructor, argName=None):
        ...
        b = CommandBlock(sp)
        sp.newCommand(
            Expr("match", [self, constructor, argName, b, get_file_line_no()])
        )
            ...
            try:
                ...
                result = Expr(
                    ...
                    [
                        self,
                        name,
                        "None" if message is None else spExpr(message),
                        get_file_line_no(),
                    ],
                )
    ...
    def push(self, other):
        return sp.set(self, cons(spExpr(other), self))
...
def open_chest(chest_key, chest, time):
    return Expr(
        "open_chest",
        [spExpr(chest_key), spExpr(chest), spExpr(time), get_file_line_no()],
    )
...
def spExpr(x, context="expression"):
    ...
    if isinstance(x, Lambda):
        if debug:
            ...
            alert("Lambda")
        return x.f
    if isinstance(x, PrivateLambda):
        if debug:
            alert(x)
            alert("PrivateLambda")
        return Expr("private", [x.name, get_file_line_no()])
...
def set_record_layout(expression, layout):
    raise Exception(
        "sp.set_record_layout is deprecated. Please use a command like sp.set_type(.., sp.TRecord(...).loyout(..))"
    )


def set_variant_layout(expression, layout):
    raise Exception(
        "sp.set_variant_layout is deprecated. Please use a command like sp.set_type(.., sp.TVariant(...).loyout(..))"
    )


def set_type_record_layout(expression, layout):
    raise Exception(
        "sp.set_type_record_layout is deprecated. Please use a command like sp.set_type(.., sp.TRecord(...).loyout(..))"
    )


def set_type_variant_layout(expression, layout):
    raise Exception(
        "sp.set_type_variant_layout is deprecated. Please use a command like sp.set_type(.., sp.TVariant(...).loyout(..))"
    )
...
class TBounded(TType):
    ...
    def export(self):
        return "(bounded %s %s %s)" % (
            self.t.export(),
            str(self.final),
            " ".join(spExpr(case).export() for case in self.cases),
        )
...
class TTuple(TType):
    ...
    def export(self):
        return "(tuple %s)" % (
            " ".join([sp.types.conv(i).export() for i in self.types])
        )
...
class TLambda(TType):
    def __init__(self, t1, t2, with_storage=None, with_operations=False):
...
class Data:
    def __getattr__(self, attr):
        if "__" in attr:
            raise AttributeError("")
        return Expr("attr", [Expr("data", []), attr, get_file_line_no()])
...
class Sp:
    ...
    def newCommand(self, command, name=None):
        if hasattr(self, "mb") and self.mb is not None:
            ...
            raise Exception(
                "New command outside of contract %s:\n%s"
                % (pretty_line_no(), str(command))
            )
    ...
    def defineLocal(self, local, name, value, mutable):
        self.newCommand(Expr("defineLocal", [name, value, mutable, get_file_line_no()]))
...
class Contract:
    ...
    def private_lambda(self, name, with_storage, with_operations, f):
    ...
    def private_variable(self, name, v):
        self.init_internal()
        self.private_variables.append((name, spExpr(v)))
        return Expr("private", [name, get_file_line_no()])

    def onoffchain_view(self, name, f, pure, doc, lineNo, kind):
        ov = OnOffchainView(name, f, pure, doc, lineNo, kind)
        self.init_internal()
        ...
        class View:
            ...
            def __call__(self, params=unit):
                ...
                ).open_some("View %s is invalid!" % self.ov.name)
    ...
    def init_internal(self):
        ...
        if not hasattr(self, "messages"):
            self.messages = {}
        if not hasattr(self, "private_variables"):
    ...
    def init_storage(self, arg=None, **kwargs):
        ...
        if self.storage is not None:
            raise Exception(
                "self.init, self.init_storage error. Storage already set on contract. Please use self.update_initial_storage. %s"
                % (pretty_line_no())
            )
    ...
    def update_initial_storage(self, arg=None, **kargs):
        ...
        if arg is not None and pyLen(kargs):
            raise Exception(
                "self.init, self.init_storage, and self.update_initial_storage use either one arg or **kwargs syntax, not both."
            )
        if arg is not None:
            ...
            if self.storage is not None:
                if not isinstance(self.storage, record):
                    raise Exception(
                        "self.init, self.init_storage, and self.update_initial_storage cannot update a non record with record fields."
                    )
    ...
    def addMessage(self, addedMessage):
        ...
        if nargs == 0:
            raise Exception(
                "Entry point '%s' is missing a self parameter (line %s)."
            ...
            args = [
                Expr("attr", [params, a, get_file_line_no()])
                for a in addedMessage.args[1:]
            ]
    ...
    def collectMessages(self):
        ...
        for f in dir(self):
            attr = getattr(self, f)
            if isinstance(attr, PrivateLambda):
                attr._l = self.private_lambda(
                    attr.name, attr.with_storage, attr.with_operations, attr.f
                )
            if isinstance(attr, OnOffchainView):
                setattr(
                    self,
                    attr.name,
                    self.onoffchain_view(
                        attr.name, attr.f, attr.pure, attr.doc, attr.line_no, attr.kind
                    ),
                )
        for f in dir(self):
            if f == "__setattr__":
    ...
    def export(self):
        ...
        for message in self.messages.values():
            if message.addedMessage.name is not None:
                ...
                if name in messages:
                    raise Exception(
                        "Two entry points are called %s (line %s)"
                        % (name, message.addedMessage.line_no.export())
                    )
        ...
        result = (
            ...
            % (
                ...
                (
                    " ".join(
                        ...
                        % (
                            ...
                            str(v.lazify),
                            str(v.lazy_no_code),
                            str(pyLen(v.addedMessage.args) > 1),
                            v.line_no.export(),
                            v.export(),
                        )
                ...
                (
                    " ".join(
                        "(" + " ".join(str(x) for x in call) + ")"
                        for call in self.flags
                    )
                ),
                (
                    " ".join(
                        "(%s %s)" % (name, variable.export())
                        for (name, variable) in self.private_variables
                    )
                ),
                (" ".join(ov.exportDef(self) for ov in self.views)),
                (
                    self.entry_points_layout
                    if self.entry_points_layout is not None
                    else "()"
                ),
                (
                    " ".join(
                        "(%s %s)" % (k, spExpr(v).export())
                        for (k, v) in self.initial_metadata.items()
                    )
                ),
                (
                    spExpr(self._initial_balance).export()
...
class AddedMessage:
    ...
    def __call__(self, *args, **kargs):
        if self.contract.messages_collecting:
            raise Exception(
                "Cannot call an entry_point '%s' from another entry_point."
                % (self.name)
            )
...
def entry_point(
    f=None, name=None, line_no=None, private=False, lazify=None, lazy_no_code=None
):
...
class PrivateLambda:
    def __init__(self, name, with_storage, with_operations, wrap_call, f, lineNo):
        self.name = name
        self.with_storage = with_storage
        self.with_operations = with_operations
        ...
        self.f = f
        self.line_no = lineNo
        self._e = Expr("private", [self.name, self.line_no])
    ...
    def __call__(self, *args):
        if pyLen(args) == 0:
            ...
            raise Exception("Private lambda got more than one argument.")
        return compute(self._e(x)) if self.wrap_call else self._e(x)
...
def private_lambda(
    name=None, with_storage=None, with_operations=False, wrap_call=False
):
    def transform(f):
        nonlocal name
        if name is None:
...
def global_lambda(f, name=None):
    print(
        f"'global_lambda' is deprecated. Please use 'private_lambda' instead. {pretty_line_no()}"
    )
    if name is None:
        name = f.__name__
    ...
    return private_lambda(
        name=name, with_storage=None, with_operations=False, wrap_call=False
    )(g)


def offchain_view(pure=False, doc=None, name=None):
    def apply_offchain_view(f):
        name_ = name if name is not None else f.__name__
        return OnOffchainView(name_, f, pure, doc, get_file_line_no(), "offchain")

    return apply_offchain_view


def onchain_view(pure=False, doc=None, name=None):
    def apply_onchain_view(f):
        name_ = name if name is not None else f.__name__
        return OnOffchainView(name_, f, pure, doc, get_file_line_no(), "onchain")
...
def sub_entry_point(f, name=None):
    print(
        f"'sub_entry_point' is deprecated. Please use 'private_lambda' instead. {pretty_line_no()}"
    )
    return private_lambda(
        name=None, with_storage="read-write", with_operations=True, wrap_call=True
    )(f)
...
def get_and_update(map, key, value=none):
    return match_pair(
        Expr(
            "get_and_update",
            [spExpr(map), spExpr(key), spExpr(value), get_file_line_no()],
        )
    )
...
def slice(expression, offset, length):
    return Expr(
        "slice",
        [spExpr(offset), spExpr(length), spExpr(expression), get_file_line_no()],
    )
...
class Lambda:
    def __init__(
        ...
        self.contract_self = contract_self
    ...
    def collectLambda(self, f):
        ...
        sp.mb.currentBlock = commands
        param = Expr(
            "lambdaParams", [self.id, self.params, get_file_line_no(), self.tParams]
        )
        if self.contract_self is None:
            r = f(param)
        else:
            import inspect
            ...
            nargs = pyLen(args)
            if nargs == 1:
                r = f(self.contract_self)
            elif nargs == 2:
                r = f(self.contract_self, param)
            else:
                raise Exception(
                    "More than one argument given in private lambda definition."
                )
        if self.auto_result:
            if r is not None:
                result(r)
            ...
            raise Exception(
                "Please use 'sp.result' instead of 'return' in SmartPy functions."
            )
        if self.with_storage not in [None, "read-write", "read-only"]:
            raise Exception(
                'with_storage parameter can only be None, "read-only" or "read-write".'
...
def build_lambda(
    ...
    return Lambda(
        f, with_storage, with_operations, params, tParams, auto_result, contract_self
    )


class Local:
    def __init__(self, name, value, mutable, t=None):
        t = sp.types.conv(t)
        sp.defineLocal(self, name, spExpr(value), mutable)
        self.init = False
        self.name = name
        self.val = value
        self.t = t
...
def local(name, value, t=None, mutable=True):
    return Local(name, set_type_expr(value, t) if t else value, mutable, t)


def compute(expression):
    id = get_id_from_line_no()
    if id == sp.last_compute_id:
        id = id + "i"
    sp.last_compute_id = id
    return local("compute_%s" % id, expression).value
...
def sapling_test_transaction(source, target, amount, memo_size):
    ...
    return Expr(
        ...
        [memo_size, source, target, str(amount), get_file_line_no()],
    )
...
def contract(t, address, entry_point="", line_no=None):
    ...
    return Expr(
        "contract",
        [entry_point, sp.types.conv(t), spExpr(address), get_file_line_no(line_no)],
    )
...
def create_contract_operation(contract, storage=None, amount=tez(0), baker=None):
    ...
    x = local(
        "create_contract_%s" % get_id_from_line_no(),
        Expr(
            "create_contract",
            [
...
def set_type_expr(expression, t):
    result = Expr(
        "type_annotation", [spExpr(expression), sp.types.conv(t), get_file_line_no()]
    )
    return result
...
def set_type(expression, t):
    result = Expr(
        "set_type", [spExpr(expression), sp.types.conv(t), get_file_line_no()]
    )
    sp.newCommand(result)
    return Expr(
        "invalid",
        [
            "Invalid expression, sp.set_type should no longer be used as an expression, please use sp.set_type_expr instead. %i"
            % (get_file_line_no().line_no)
        ],
    )
...
def set_entry_point(ep_name, new_ep):
    sp.newCommand(
        Expr("set_entry_point", [ep_name, spExpr(new_ep), get_file_line_no()])
    )
...
class Dynamic_contract:
    ...
    def call(self, entry_point, parameters):
        res = ExecMessage(
            None, entry_point, [parameters], None, contractId=self.contractId
        )
...
class Scenario:
    ...
    def pp(self):
        ...
        if window.in_browser:
            ...
            messages = {"scenario": self.messages, "shortname": "aaa", "kind": "test"}
...
def add_test(name, shortname=None, profile=False, is_default=True):
    return add_target(
        name, shortname=shortname, profile=profile, is_default=is_default, kind="test"
    )


def add_simulation_target(
    contract,
    name="Simulation",
    shortname=None,
    profile=False,
    ...
    return add_test(
        name=name, shortname=shortname, profile=profile, is_default=is_default
    )(test)


def add_compilation_target(
    name,
    contract,
    storage=None,
    parameters=[],
    ...
    add_target(
        name=name, shortname=shortname, is_default=is_default, kind="compilation"
    )(f)


def add_expression_compilation_target(
    name, expression, shortname=None, is_default=False, flags=[]
):
    def f():
        scenario = test_scenario()
        for flag in flags:
...
def compile_contract(contract, target_directory, name="contract"):
    ...
    def __call__(self, x):
        ...
        try:
            ...
            if False:
                ...
                if self.app is None:
                    self.app = self.contract.private_variable(
                        "get_error", build_lambda(self.get)
                    )
...
def catch_exception(expression, t=None):
    return Expr(
        "catch_exception", [spExpr(expression), sp.types.conv(t), get_file_line_no()]
    )
...
def ticket(content, amount):
    line_no = get_file_line_no()
    x = local(
        "ticket_%i" % line_no.line_no,
        Expr("ticket", [spExpr(content), spExpr(amount), line_no]),
    )
    return x.value
...
class Modify_record(CommandBlock):
    ...
    def __setattr__(self, attr, value):
        if (
            ...
            and not attr in ["sp", "currentBlock"]
        ):
            sp.set(getattr(self, attr), value)
        else:
            object.__setattr__(self, attr, value)
    ...
    def export(self):
        return (
            Expr(
                "modify_record",
                [self.line_no, self._x, self._var, self.commands]
                + [Expr("binding", b) for b in self.bindings],
            )
        ).export()
...
class ContractTyped:
    ...
    def export(self):
        return Expr(
            "contract_typed", [self.contractId, self.entry_point, self.line_no]
        ).export()
...
class utils:
    ...
    def cube(xs, tkey=TIntOrNat, tvalue=None):
        return set_type_expr(
            utils.cube_raw(xs), TMap(tkey, TMap(tkey, TMap(tkey, tvalue)))
        )
    ...
    def view(t, message=None):
        def app(f):
            ...
            def ep(self, params):
                ...
                transfer(view_result.value, tez(0), snd(params), line_no=line_no)
    ...
    def bytes_of_string(s):
        if not (isinstance(s, str)):
            raise Exception(
                "sp.utils.bytes_of_string must be applied to constant strings and got (%s)"
                % (str(s))
            )
    ...
    @staticmethod
    def lambda_with_operations(f, params="", tParams=None):
        def f_wrapped(x):
        ...
        return build_lambda(f_wrapped, params, tParams)

    @staticmethod
    def lambda_operations_only(f, params="", tParams=None):
        def f_wrapped(x):
            ops = local("__operations__", [], t=TList(TOperation))
            f(x)
            result(ops.value)
...
class io:
    ...
    def import_stored_contract(name):
        ...
        if not code:
            raise Exception(
                "Could not import_stored_contract (" + name + "). It doesn't exist."
            )
