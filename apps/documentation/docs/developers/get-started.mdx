...
[Deploy with SmartPy IDE](https://smartpy.io/ide?cid=QmaEB9fGwMfY9ksLtpuWEAPXkSDzA9NttNqQGN21JXoF6r&k=40a7361fcc5f545a1234) (Use Ghostnet)
...
- Then wait for a snapshot with a `block_number` greater or equal to the `block_number` of the `increment()` transaction. You can do that by querying the latest snapshot as shown in the example below.
  ...
  const TEZOS_RPC = "http://tezos-ghostnet-node-1.diamond.papers.tech";
