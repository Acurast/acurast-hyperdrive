...
def bits_of_bytes(self, b):
    ...
    with sp.for_("x", sp.range(0, sp.len(b), 1)) as x:
        bits.value += self.data.bytes_to_bits[sp.slice(b, x, 1).open_some("Out of bonds")]
...
def bytes_of_bits(self, b):
    ...
    with sp.for_("x", sp.range(sp.to_int(sp.len(b)), 8, -8)) as x:
        _bytes.value = self.data.bits_to_bytes[sp.slice(b, sp.as_nat(x - 8), 8).open_some("Out of bonds")] + _bytes.value
