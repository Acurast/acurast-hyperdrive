...
contract('IBCF_Bridge', async ([_, primary]) => {
    ...
    before('Deploy contracts', async () => {
        validator = await IBCF_Validator.new(primary, 1, chain_id, 5, [primary], { from: primary });
    ...
    it('Call unwrap', async function() {
        ...
        await validator.submit_state_root(snapshot, valid_merkle_root, { from: primary })
        ...
        await expectsFailure(async () => await bridge.unwrap(snapshot, key, value, proof), "Expected error (invalid nonce).");
    });
})
