...
def test_encoder():
    ...
    # Test lambdas
    scenario.verify(
        sp.build_lambda(Encoder.encode_length)((1, STRING_SHORT_START))
        == sp.bytes("0x81")
    )
    scenario.verify(
    ...
    scenario.verify(
        sp.build_lambda(Encoder.encode_string)("TEXT") == sp.bytes("0x8454455854")
    )
    scenario.verify(
        sp.build_lambda(Encoder.encode_list)(
...
def test_decoder():
    ...
    # Test lambdas
    scenario.verify(
        sp.build_lambda(Decoder.length)(sp.bytes("0xcb87038d7ea4c67fff82ffff")) == 12
    )
    scenario.verify(
    ...
    scenario.verify(
        sp.build_lambda(Decoder.decode_string)(sp.bytes("0x8454455854")) == "TEXT"
    )
    scenario.verify_equal(
        sp.build_lambda(Decoder.decode_list)(sp.bytes("0xcb87038d7ea4c67fff82ffff")),
