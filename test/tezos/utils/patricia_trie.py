...
from contracts.tezos.utils.misc import split_common_prefix, chop_first_bit
...
def test_split_common_prefix():
    ...
    # Test lambda
    result = scenario.compute(sp.build_lambda(split_common_prefix)((sp.record(data=15, length=4), sp.record(data=7, length=3))))
    expected_result = sp.record(
        prefix = sp.record(data=7, length=3),
        suffix = sp.record(data=1, length=1)
    )
...
def test_chop_first_bit():
    ...
    label = sp.record(data=103467121227129887621618079513505806301059647461257568066320101310811728706554, length=256)
