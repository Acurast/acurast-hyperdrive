...
const ETH_BRIDGE_ADDRESS =
  process.argv[2] || "0x0C2dBba45E207619B79Fd7946496fbEc1E66fA8C";

const ACCOUNT_STATE_ROOT_INDEX = 2;

const ETH_BLOCK_NUMBER = 7976909;
const tezos_address =
  "0x050a000000160000eaeec9ada5305ad61fc452a5ee9f7d4f55f80467";
...
(async () => {
  ...
  const storageProof = account.storageProof[0].proof.map((x) =>
    Buffer.from(x.slice(2), "hex")
  );
  ...
  try {
    ...
    const storage = await trie.verifyProof(
      accountStateRoot,
      Buffer.from(Web3.utils.sha3(storageSlot).slice(2), "hex"),
      storageProof
    );
