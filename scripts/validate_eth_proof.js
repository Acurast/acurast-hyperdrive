...
const ETH_BRIDGE_ADDRESS = process.argv[2] || '0xF8D0dcb8F3Af348586360971b3561B3b139a2929';
...
const destinationSlot = Web3.utils.sha3("0x" + hexNonce + destinationRegistryIndex);
...
(async () => {
    const [block, account] = await Promise.all([
        ...
        web3_sdk.eth.getProof(ETH_BRIDGE_ADDRESS, [destinationSlot, amountSlot], ETH_BLOCK_NUMBER)
    ])
    ...
    const destinationProof = account.storageProof[0].proof.map(x => Buffer.from(x.slice(2), 'hex'))
    const amountProof = account.storageProof[1].proof.map(x => Buffer.from(x.slice(2), 'hex'))
    ...
    try {
        ...
        // Validate account
        if (!value || !value.equals(Buffer.from(RLP.encode([Number(account.nonce), Number(account.balance), account.storageHash, account.codeHash]))))
            ...
            const destination = await trie.verifyProof(accountStateRoot, Buffer.from(Web3.utils.sha3(destinationSlot).slice(2), 'hex'), destinationProof)
            console.log('Destination:', destination)
            const amount = await trie.verifyProof(accountStateRoot, Buffer.from(Web3.utils.sha3(amountSlot).slice(2), 'hex'), amountProof)
