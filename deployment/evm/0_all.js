...
module.exports = async function (deployer, _network, _accounts) {
  console.log(await deployer.deploy(IBCF_Validator, administrator, 1, tezos_chain_id, 5, [
    administrator,
  ]));
  const validator = await IBCF_Validator.deployed();

  const asset = await deployer.deploy(ERC20, "TEST", "TEST")
  const bridge = await deployer.deploy(IBCF_Bridge, validator.address, asset.address);

  const crowdfunding = await deployer.deploy(IBCF_Crowdfunding);

  const output = `IBCF_Validator: ${validator.address}\n` +
  `TEST_ERC20: ${asset.address}\n` +
  `IBCF_Bridge: ${bridge.address}\n` +
  `IBCF_Crowdfunding: ${crowdfunding.address}`;

  fs.writeFileSync("__SNAPSHOTS__/evm-deployment.txt", output, { encoding: "utf-8" })
};
